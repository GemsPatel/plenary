...
jQuery(document).ready(function ($) {
    ...
    $('.plenary-tabs li').on('click', 'a', function () {
        if ($(this).hasClass('active') || $(this).attr('data-absolutetab') == '')
            return false;
        $(this).parents('.plenary-tabs').find('li a').removeClass('active');
        $(this).addClass('active');

        // mobile
        $('.plenary-tab-mob[data-absolutetab-num=' + $(this).data('absolutetab-num') + ']').parents('.plenary-tab-cont').find('.plenary-tab-mob').removeClass('active');
        $('.plenary-tab-mob[data-absolutetab-num=' + $(this).data('absolutetab-num') + ']').addClass('active');

        $($(this).attr('data-absolutetab')).parents('.plenary-tab-cont').find('.plenary-tab').css('height', '0px');
        $($(this).attr('data-absolutetab')).css('height', 'auto').hide().fadeIn();
        return false;
    });

    // Popular Products Tabs (mobile)
    $('.plenary-tab-cont').on('click', '.plenary-tab-mob', function () {
        if ($(this).hasClass('active') || $(this).attr('data-absolutetab') == '')
            return false;
        $(this).parents('.plenary-tab-cont').find('.plenary-tab-mob').removeClass('active');
        $(this).addClass('active');

        // main
        $('.plenary-tabs li a[data-absolutetab-num=' + $(this).data('absolutetab-num') + ']').parents('.plenary-tabs').find('li a').removeClass('active');
        $('.plenary-tabs li a[data-absolutetab-num=' + $(this).data('absolutetab-num') + ']').addClass('active');

        $($(this).attr('data-absolutetab')).parents('.plenary-tab-cont').find('.plenary-tab').animate({
            'height': '0px'
        }, 350);
        $($(this).attr('data-absolutetab')).animate({
            'height': $($(this).attr('data-absolutetab')).find('.plenary-tab-inner').outerHeight()+'px'
        }, 350);
...
jQuery(window).load(function(){
    ...
    // Tabs "more" button
    if ($('.plenary-tabs').length > 0) {
        if ($(window).width() > 751) {
            var menu_sections = $('.plenary-tabs');
            var menu_width = menu_sections.width();
            var menu_items_width = 0;
            menu_sections.find('> li').each(function () {
                if (!$(this).hasClass('plenary-tabs-more')) {
                    menu_items_width = menu_items_width + $(this).outerWidth(true);
                    if (menu_width < menu_items_width) {
                        $(this).addClass('plenary-tabs-other');
                        $(this).appendTo('.plenary-tabs-sub');
                    } else if ($(this).hasClass('plenary-tabs-other')) {
                        $(this).removeClass('plenary-tabs-other');
                        $(this).prependTo('.plenary-tabs-sub');
                    }
                }
            });
            if (menu_width < menu_items_width) {
                $('.plenary-tabs-more').show();
            }
        }

        $('.plenary-tabs').addClass('sections-show');
        ...
        $(window).resize(function() {
            ...
            if ($(window).width() > 751) {
                ...
                menu_sections.find('li.plenary-tabs-other').insertBefore('.plenary-tabs-more');
                menu_sections.find('li.plenary-tabs-other').removeClass('plenary-tabs-other');
            }
        });
